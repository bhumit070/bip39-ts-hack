...
function mnemonicToSeedSync(mnemonic: string, password: string) {
    const mnemonicBuffer = Buffer.from(normalize(mnemonic), 'utf8');
    const saltBuffer = Buffer.from(salt(normalize(password)), 'utf8');
    return pbkdf2_1.pbkdf2Sync(mnemonicBuffer, saltBuffer, 2048, 64, 'sha512');
}
exports.mnemonicToSeedSync = mnemonicToSeedSync;
function mnemonicToSeed(mnemonic: any, password: any) {
    return Promise.resolve().then(() => {
        const mnemonicBuffer = Buffer.from(normalize(mnemonic), 'utf8');
        const saltBuffer = Buffer.from(salt(normalize(password)), 'utf8');
...
function mnemonicToEntropy(mnemonic: any, wordlist: any) {
...
function entropyToMnemonic(entropy: any, wordlist: any) {
...
function generateMnemonic(strength: any, rng: any, wordlist: any) {
...
function validateMnemonic(mnemonic: any, wordlist: any) {
    try {
...
function setDefaultWordlist(language: any) {
    const result = _wordlists_1[language];
    if (result) {
...
function getDefaultWordlist() {
    ...
    return Object.keys(_wordlists_1.wordlists).filter((lang) => {
        ...
        return _wordlists_1.wordlists[lang].every(
            (word: any, index: any) => word === DEFAULT_WORDLIST[index],
        );
    })[0];
}
exports.getDefaultWordlist = getDefaultWordlist;
// @ts-ignore
//import * as _wordlists_2 from './_wordlists';
